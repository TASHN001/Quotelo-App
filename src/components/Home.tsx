...
import { Plus, FileText, Eye, Share2, Copy, CheckCircle, MoreVertical } from 'lucide-react';
                    ...
                    <>
                      ...
                      <div className={`absolute right-3 ${menuOpenAbove ? 'bottom-full mb-1' : 'top-full mt-1'} w-44 bg-white ${ds.radiusMd} ${ds.shadow3} z-20 overflow-hidden border border-[#e5e5ea]`}>
                        {[
                          ...
                          { label: 'Duplicate',     icon: Copy,        action: (e: React.MouseEvent) => handleDuplicate(invoice.id, e) },
                          ...(invoice.status !== 'paid' ? [{ label: 'Mark as paid', icon: CheckCircle, action: (e: React.MouseEvent) => handleMarkAsPaid(invoice.id, e) }] : []),
                        ].map(({ label, icon: Icon, action }) => (
                          <button
                            key={label}
                            onClick={e => { e.stopPropagation(); setActiveMenuId(null); (action as any)(e); }}
                            className={`w-full flex items-center gap-3 px-4 py-3 text-[15px] text-[#3c3c43] font-medium hover:bg-[#f2f2f7] ${ds.transition} border-b last:border-b-0 border-[#f2f2f7]`}
                          >
